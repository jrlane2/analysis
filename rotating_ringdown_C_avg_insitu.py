...
class extract_data(object):
    ...
    def __init__(self, data, dataspecs, savefolder, stack_spec = [0.01, 0.01, 0, 0]):
        ...
        self.meastimeproxy = self.iters*2*(self.setdrivetime + self.setresttime)
        
        #load data and kill nans
        self.dat = data
        [self.minv, self.maxv] = self.killnans2()
    ...
    def killnans(self):
        '''
        Function to remove nan-s and shenanigans.
        '''
        ...
        nanrows = np.union1d(nanrows1, nanrows2).astype(int)
        left = target
        self.check3 = 0
        if len(nanrows) > 0:
    ...
    def simple_ringdown(self):
        ...
        self.tempS = np.where((np.diff(self.TTL2) == -1)*1 == 1)[0]
        self.tempS2 = self.ift(self.time, self.tfi(self.time, self.tempS, self.minv, self.maxv)-self.k1, self.minv, self.maxv)
        ...
        for e in self.elements:
            self.stackS[e,:,2] = self.stackS[e,:,2]/(np.exp(+1j*np.angle(self.stackS[e,self.ift(self.time, self.k1 - self.time[0], self.minv, self.maxv),2])))
            ...
            self.phasestackS[e,:,1] = (self.phasestackS[e,:,1] - self.phasestackS[e,self.ift(self.time, self.k1 - self.time[0], self.minv, self.maxv) - self.minv + 1 + 1,1])/(2*np.pi)
